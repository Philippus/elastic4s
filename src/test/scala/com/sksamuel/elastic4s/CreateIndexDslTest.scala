...
class CreateIndexDslTest extends FlatSpec with MockitoSugar with OneInstancePerTest {
  ...
  "the index dsl" should "generate json to include mapping properties" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/createindex_mappings.json"))
  ...
  it should "support override built in analyzers" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/createindex_analyis.json"))
  ...
  it should "support custom analyzers, tokenizers and filters" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/createindex_analyis2.json"))
    val req = create.index("users").analysis(
  ...
  it should "supported nested fields" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_nested.json"))
    val req = create.index("users").shards(2).mappings(
      "tweets" as(
  ...
  it should "generate json to override index settings when set" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/createindex_settings.json"))
    val req = create index "users" shards 3 replicas 4
    assert(json === mapper.readTree(req._source.string))
  }

  it should "support inner objects" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_inner_object.json"))
  ...
  it should "support disabled inner objects" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_inner_object_disabled.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as(
        "person" inner(
  ...
  it should "support multi field type" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_multi_field_type_1.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as (
        "name" multi(
  ...
  it should "support multi field type with path" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_multi_field_type_2.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as(
  ...
  it should "support copy to a single field" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_copy_to_single_field.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as(
  ...
  it should "support copy to multiple fields" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_copy_to_multiple_fields.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as(
  ...
  it should "support completion type" in {
    val json = mapper.readTree(getClass.getResource("/json/createindex/mapping_completion_type.json"))
    val req = create.index("tweets").shards(2).mappings(
      "tweet" as (
          "name" typed StringType index "analyzed",
