...
/** A Pekko [[org.apache.pekko.stream.scaladsl.Source]], that publishes documents using an elasticsearch scroll cursor.
  * The initial query must be provided to the source, and there are helpers to create a query for all documents in an
  * index.
  *
  * @param client
...
class ElasticSource(client: ElasticClient, settings: SourceSettings)(implicit ec: ExecutionContext)
  ...
  private implicit val scrollHandler: Handler[SearchScrollRequest, SearchResponse]          =
