...
class SearchHttpTest extends WordSpec with DiscoveryLocalNodeProvider with ElasticDsl with Matchers {
  ...
  "a search query" should {
    ...
    "support match all query" in {
      ...
      }.await.get.totalHits shouldBe 6
    }
    "support sorting in a single type" in {
      http.execute {
        search("chess" / "pieces") query matchAllQuery() sortBy fieldSort("name")
      }.await.get.hits.hits.map(_.sourceField("name")) shouldBe Array("bishop", "king", "knight", "pawn", "queen", "rook")
    }
    "support explain" in {
      http.execute {
        search("chess").explain(true).matchAllQuery().limit(2)
      }.await.get.hits.hits.head.explanation.isDefined shouldBe true
    }
    "support limits" in {
      http.execute {
        search("chess").matchAllQuery().limit(2)
      }.await.get.size shouldBe 2
    }
    "support unmarshalling through a HitReader" in {
      ...
      }.await.get.to[Piece] shouldBe Vector(Piece("bishop", 3, 2), Piece("king", 0, 1), Piece("knight", 3, 2))
    }
    "support source includes" in {
      http.execute {
        search("chess") query matchAllQuery() sourceInclude "count"
      }.await.get.hits.hits.map(_.sourceAsString).toSet shouldBe Set("{\"count\":1}", "{\"count\":2}", "{\"count\":8}")
    }
    "support source excludes" in {
      http.execute {
        search("chess") query matchAllQuery() sourceExclude "count"
      }.await.get.hits.hits.map(_.sourceAsString).toSet shouldBe Set("{\"name\":\"pawn\",\"value\":1}", "{\"name\":\"knight\",\"value\":3}", "{\"name\":\"king\",\"value\":0}", "{\"name\":\"rook\",\"value\":5}", "{\"name\":\"queen\",\"value\":10}", "{\"name\":\"bishop\",\"value\":3}")
    }
    "support constantScoreQuery" should {
      ...
      "not throw npe on empty hits for safeTo" in {
        ...
        }.await.get.safeTo[Piece].size shouldBe 0
      }
      "support search hits without null on no hits" in {
        ...
        }.await.get.hits.hits.isEmpty shouldBe true
      }
