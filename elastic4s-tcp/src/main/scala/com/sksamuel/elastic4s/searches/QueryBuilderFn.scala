...
object QueryBuilderFn {
  def apply(query: QueryDefinition): QueryBuilder = query match {
    ...
    case q: IdQuery                       => IdQueryBuilderFn(q)
    ...
    case q: PrefixQuery                   => PrefixQueryBuilderFn(q)
    ...
    // Not implemented
    case ni =>
      throw new NotImplementedError(s"Query ${ni.getClass.getName} has not yet been implemented for the TCP client.")
  }
}
