...
object EnumConversions {
  ...
  def indicesopts(opts: IndicesOptions): org.elasticsearch.action.support.IndicesOptions =
    org.elasticsearch.action.support.IndicesOptions
      .fromOptions(opts.ignoreUnavailable, opts.allowNoIndices, opts.expandWildcardsOpen, opts.expandWildcardClosed)

  implicit def settings(map: Map[String, Any]): Settings =
    map
      .foldLeft(Settings.builder) {
        case (settings, (key, value)) =>
          settings.put(key, value.toString)
      }
      .build()

  implicit def simpleQueryStringFlag(
  ...
  implicit def percentilesMethod(
    ...
    percentilesMethod match {
      ...
      case PercentilesMethod.TDigest =>
        org.elasticsearch.search.aggregations.metrics.percentiles.PercentilesMethod.TDIGEST
    }
  ...
  implicit def stringDistance(
    suggestMode: StringDistanceImpl
  ): org.elasticsearch.search.suggest.term.TermSuggestionBuilder.StringDistanceImpl =
    suggestMode match {
      case StringDistanceImpl.DAMERAU_LEVENSHTEIN =>
        org.elasticsearch.search.suggest.term.TermSuggestionBuilder.StringDistanceImpl.DAMERAU_LEVENSHTEIN
  ...
  implicit def geo(point: GeoPoint): org.elasticsearch.common.geo.GeoPoint =
    new org.elasticsearch.common.geo.GeoPoint(point.lat, point.long)
  ...
  implicit def distanceUnit(distanceUnit: DistanceUnit): org.elasticsearch.common.unit.DistanceUnit =
    distanceUnit match {
  ...
  implicit def geoValidationMethod(method: GeoValidationMethod): org.elasticsearch.index.query.GeoValidationMethod =
    method match {
  ...
  implicit def scoreMode(scoreMode: QueryRescoreMode): org.elasticsearch.search.rescore.QueryRescoreMode =
    scoreMode match {
  ...
  implicit def scriptSortType(
    sortType: ScriptSortType
  ): org.elasticsearch.search.sort.ScriptSortBuilder.ScriptSortType = sortType match {
  ...
  implicit def collectMode(
    mode: SubAggCollectionMode
  ): org.elasticsearch.search.aggregations.Aggregator.SubAggCollectionMode = mode match {
    case SubAggCollectionMode.BreadthFirst =>
      org.elasticsearch.search.aggregations.Aggregator.SubAggCollectionMode.BREADTH_FIRST
    case SubAggCollectionMode.DepthFirst =>
      org.elasticsearch.search.aggregations.Aggregator.SubAggCollectionMode.DEPTH_FIRST
  }
