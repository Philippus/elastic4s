...
The latest release is 1.5.8 which is compatible with Elasticsearch 1.5.x. There are releases for both Scala 2.10 and Scala 2.11. For releases that are compatible with earlier versions of Elasticsearch,
...
```groovy
compile 'com.sksamuel.elastic4s:elastic4s_2.11:1.5.8'
```

For SBT users simply add:

```scala
libraryDependencies += "com.sksamuel.elastic4s" %% "elastic4s" % "1.5.8"
```
