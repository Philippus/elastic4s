...
/** Uri used to connect to an Elasticsearch cluster. The general format is
  ...
  *
  * Multiple host:port combinations can be specified, seperated by commas. Options can be specified using standard uri
  * query string syntax, eg cluster.name=superman
  *
