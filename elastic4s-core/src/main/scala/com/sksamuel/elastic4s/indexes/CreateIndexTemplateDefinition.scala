...
case class TemplateAlias(name: String, filter: Option[QueryDefinition] = None, routing: Option[String] = None) {
...
case class CreateIndexTemplateDefinition(name: String,
  ...
  def analysis(analyzers: Iterable[AnalyzerDefinition],
               normalizers: Iterable[NormalizerDefinition]): CreateIndexTemplateDefinition =
    analysis match {
  ...
  def normalizers(first: NormalizerDefinition, rest: NormalizerDefinition*): CreateIndexTemplateDefinition =
    analysis(Nil, first +: rest)
  def normalizers(normalizers: Iterable[NormalizerDefinition]): CreateIndexTemplateDefinition =
