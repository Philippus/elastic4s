...
trait SearchTemplateApi {
  ...
  @deprecated("Use the stored scripts api to store templates", "6.0.0")
  def putSearchTemplate(name: String, body: String): PutSearchTemplateDefinition =
    PutSearchTemplateDefinition(name, none, body.some)
  ...
  class TemplateSearchExpectsName(indexesAndTypes: IndexesAndTypes) {
    ...
    class TemplateSearchExpectsParams(indexesAndTypes: IndexesAndTypes, name: String) {
      def params(params: Map[String, Any]): TemplateSearchDefinition =
        TemplateSearchDefinition(indexesAndTypes, name, params)
    }
  }
}
