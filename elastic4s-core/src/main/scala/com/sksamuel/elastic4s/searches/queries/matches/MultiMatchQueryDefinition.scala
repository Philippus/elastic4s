...
case class MultiMatchQueryDefinition(text: String,
  ...
  def fields(_fields: Iterable[String]): MultiMatchQueryDefinition =
    copy(fields = _fields.map(FieldWithOptionalBoost(_, None)).toSeq)
  def field(name: String, boost: Double): MultiMatchQueryDefinition =
    copy(fields = fields :+ FieldWithOptionalBoost(name, boost.some))
  def fields(fields: Map[String, Double]): MultiMatchQueryDefinition =
