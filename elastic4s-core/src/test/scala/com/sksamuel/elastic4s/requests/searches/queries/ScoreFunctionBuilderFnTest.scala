...
class ScoreFunctionBuilderFnTest extends AnyFlatSpec with JsonSugar {
  ...
  "a score dsl" should "generate correct json for a linear decay function scorer" in {
    val req     =
      linearScore("myfield", "1 2", "2km").offset("200m").decay(0.1).weight(0.1).multiValueMode(MultiValueMode.Min)
  ...
  it should "generate correct json for a gaussian decay function scorer" in {
    val req     =
      gaussianScore("myfield", "1 2", "3km").offset("1km").decay(0.2).weight(0.2).multiValueMode(MultiValueMode.Min)
  ...
  it should "generate correct json for an exponential decay function scorer" in {
    val req     =
      exponentialScore("myfield", "1 2", "4km").offset("100m").decay(0.4).weight(0.3).multiValueMode(MultiValueMode.Min)
    val builder = queries.ScoreFunctionBuilderFn(req)
    val actual  = builder.string
