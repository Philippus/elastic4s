...
class PutPipelineRequestHandlerTest extends AnyFlatSpec with IngestHandlers with Matchers {
  ...
  it should "build a pipeline request with a langdetect processor" in {
    val req         = PutPipelineRequest(
      "langdetect-pipeline",
      "A pipeline to do whatever",
      CustomProcessor("langdetect", "{\"field\": \"my_field\", \"target_field\": \"language\" }")
    )
    val correctJson = XContentFactory.parse(
      """{"description":"A pipeline to do whatever","processors":[{"langdetect":{"field": "my_field", "target_field": "language" }}]}"""
    ).string
  ...
  it should "build a pipeline request with multiple processors" in {
    val req         = PutPipelineRequest(
      "multi-pipeline",
      "A pipeline with multiple processors",
      Seq(
    ...
    val correctJson = XContentFactory.parse(
      """{"description":"A pipeline with multiple processors","processors":[{"geoip":{"field":"ip"}},{"langdetect":{"field": "my_field", "target_field": "language" }}]}"""
    ).string
