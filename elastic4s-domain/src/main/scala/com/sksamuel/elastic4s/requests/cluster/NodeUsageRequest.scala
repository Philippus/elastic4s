...
case class NodeUsageRequest(nodeId: Option[String] = None,
  ...
  def masterTimeout(timeout: Duration): NodeUsageRequest = copy(masterTimeout = (timeout.toNanos + "nanos").some)
  def masterTimeout(timeout: String): NodeUsageRequest = copy(masterTimeout = timeout.some)

  def timeout(timeout: Duration): NodeUsageRequest = copy(timeout = (timeout.toNanos + "nanos").some)
