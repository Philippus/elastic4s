...
case class NodeUsageRequest(
  ...
  def masterTimeout(timeout: Duration): NodeUsageRequest = copy(masterTimeout = s"${timeout.toNanos}nanos".some)
  def masterTimeout(timeout: String): NodeUsageRequest   = copy(masterTimeout = timeout.some)

  def timeout(timeout: Duration): NodeUsageRequest = copy(timeout = s"${timeout.toNanos}nanos".some)
