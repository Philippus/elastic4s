...
case class ForceMergeRequest(
  ...
  /** Should the optimization only expunge deletes from the index, without full optimization. Defaults to full
    * optimization (<tt>false</tt>).
    */
  def onlyExpungeDeletes(expunge: Boolean): ForceMergeRequest = copy(onlyExpungeDeletes = expunge.some)

  def waitForCompletion(waitForCompletion: Boolean): ForceMergeRequest =
