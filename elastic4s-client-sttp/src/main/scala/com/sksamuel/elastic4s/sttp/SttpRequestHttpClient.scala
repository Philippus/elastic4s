...
)(implicit sttpBackend: SttpBackend[F, Any]) extends HttpClient[F] {
...
object SttpRequestHttpClient {
  ...
  private implicit def defaultSttpBackend: SttpBackend[Future, Any] = AsyncHttpClientFutureBackend()
  ...
  def apply(clientUri: ElasticsearchClientUri)(implicit ec: ExecutionContext): SttpRequestHttpClient[Future] = {
    ...
    implicit val futureMonad: MonadError[Future] = SttpRequestHttpClient.futureMonad
