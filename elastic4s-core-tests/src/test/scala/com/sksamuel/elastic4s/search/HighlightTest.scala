  ...
  "highlighting" should {
    "highlight selected words" ignore {
      val resp = client.execute {
        searches in "intros" / "tv" query "frontier" highlighting highlight("text")
      }.await
    ...
    "use fragment size" ignore {
      val resp = client.execute {
        searches in "intros" / "tv" query "new" highlighting (
    ...
    "use no match size" ignore {
      val resp = client.execute {
        searches in "intros" / "tv" query "trek" highlighting (
    ...
    "use pre tags" ignore {
      val resp = client.execute {
        searches in "intros" / "tv" query "frontier" highlighting (
